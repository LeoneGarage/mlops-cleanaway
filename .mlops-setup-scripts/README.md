...
* Be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://adb-8460772231412889.9.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-8460772231412889.9.azuredatabricks.net#setting/accounts). If
...
* Determine the Azure AAD tenant (directory) ID and subscription associated with your staging and prod workspaces,
  ...
  the subscription. To do this:
    ...
    2. Search for your staging and prod workspaces by name to verify that they're part of the current directory. If you don't know the workspace names, you can log into the
       [staging workspace](https://adb-8460772231412889.9.azuredatabricks.net) and [prod workspace](https://adb-8460772231412889.9.azuredatabricks.net) and use the
...
* Configure a Databricks CLI profile for your staging workspace by running
  ``databricks configure --token --profile "mlops-cleanaway-staging" --host https://adb-8460772231412889.9.azuredatabricks.net``, 
...
* Create a [Databricks REST API token](https://learn.microsoft.com/azure/databricks/dev-tools/api/latest/authentication#generate-a-personal-access-token)
  in the staging workspace ([link](https://adb-8460772231412889.9.azuredatabricks.net#setting/account))
  and paste the value into the prompt.
* Configure a Databricks CLI for your prod workspace by running ``databricks configure --token --profile "mlops-cleanaway-prod" --host https://adb-8460772231412889.9.azuredatabricks.net``
* Create a Databricks REST API token in the prod workspace ([link](https://adb-8460772231412889.9.azuredatabricks.net#setting/account)).
  and paste the value into the prompt
